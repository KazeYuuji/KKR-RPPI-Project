...
export const GET: APIRoute = async ({ params }) => {
  ...
  page.drawRectangle({ x: 30, y: 370, width: width - 60, height: 1, color: rgb(0.85, 0.85, 0.88) });
  ...
  for (let i = 0; i < fields.length; i++) {
    page.drawRectangle({ x: 30, y: yPos - 8, width: width - 60, height: 32, color: i % 2 === 0 ? white : undefined });
    page.drawText(fields[i].label, { x: 35, y: yPos, size: 8, font: fontReg, color: gray });
    page.drawText(fields[i].value, { x: 35, y: yPos - 14, size: 11, font, color: dark });
    yPos -= 42;
  }
