...
const POST = async ({ request }) => {
  ...
  try {
    ...
    if (action === "verify") {
      ...
      const r2 = await db.execute("SELECT * FROM registrants WHERE id = ?", [id]);
      return new Response(JSON.stringify({ registrant: r2.rows[0] }), { status: 200, headers: { "Content-Type": "application/json" } });
    }
    if (action === "checkin") {
